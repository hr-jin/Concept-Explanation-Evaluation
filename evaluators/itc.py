...
class InputTopicCoherenceEvaluator(nn.Module, BaseEvaluator):
    ...
    def get_metric(self, eval_tokens, concept=None, concept_idx=-1, pmi_type='uci'):
        most_critical_tokens, most_critical_token_idxs = self.get_most_critical_tokens(eval_tokens, concept, concept_idx)
        ...
        logger.info('Input Topic Coherence Metric ({}): {:.4f}'.format(pmi_type, final_pmi))    
        return final_pmi
