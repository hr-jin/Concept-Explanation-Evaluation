...
_DEFAULT_TREEBANKS_ROOT = path.join(cfg['data_dir'], "ud/ud-treebanks-v2.1")
...
print('TREEBANK_PATH:', treebank_path)
limit_number = None
bert_model = args.bert # modified
fasttext_model = path.join(args.EMBEDDINGS_ROOT, "fasttext", args.fasttext) if args.fasttext else None
skip_existing = args.skip_existing
print('SKIL_EXISTING:', skip_existing)
device = 'cpu'
if args.use_gpu:
    print("Using GPU")
    device = 0
...
for f in os.listdir(treebank_path):
    if path.isfile(path.join(treebank_path, f)) and "-um-" in f and f.endswith(".conllu"):
        ...
        with open(full_path, "r") as h:
            ...
            tokenizer = BertTokenizer.from_pretrained("/data/zhihao/bert-base-multilingual-cased/")
