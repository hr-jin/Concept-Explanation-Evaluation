...
buffer_mult=10
dict_mult=4
model_to_interpret="llama-7b-chat"
model_dir="/data2/OpenLLMs/Llama-2-7b-chat-hf"     
device_list='1,4,6,7'
...
dataset_name="pile" 
dataloader='ae' # choose from ["ae", "tcav", 'conceptx_naive', 'neuron']
load_path='/data2/home/haoran/acl2024-concept/acl2024-rebuttal/data/output/model_llama-2-7b-chat_layer_16_dictSize_16384_site_resid_post/Iteration10000_Epoch1'
layer=16
...
log_str="logs/0403_fuxian_"$metric_evaluator"_layer"$layer"_topic"$topic_len"_"$return_type"_seed"$seed"_"$model_to_interpret"_data_"$dataset_name"_extr_"$extractor".log"

echo $log_str

nohup python -u main_llama2.py --load_extractor --device_list=$device_list --tokenized --topic_len=$topic_len --dataloader=$dataloader \
...
# nohup python -u main_llama2.py --device_list=$device_list --topic_len=$topic_len --dataloader=$dataloader \
#         --return_type=$return_type --use_bias_d=$use_bias_d --metric_eval_batchsize=$metric_eval_batchsize --val_freq=$val_freq \
#         --metric_evaluator=$metric_evaluator --concept_eval_batchsize=$concept_eval_batchsize \
#         --evaluator=$evaluator --load_path=$load_path  --output_dir=$output_dir --data_dir=$data_dir --extractor=$extractor \
#         --dataset_name=$dataset_name  --seq_len=$seq_len --buffer_mult=$buffer_mult --model_dir=$model_dir \
